...
describe("VersionManager", () => {
  ...
  beforeEach(() => {
    ...
    // Restore mock values that might have been modified by tests
    (Application as any).nativeApplicationVersion = "2.1.0";
    (Application as any).nativeBuildVersion = "42";
    (Application as any).applicationId = "com.test.app";
    (Application as any).applicationName = "Test App";
  ...
  describe("Native Platform (iOS/Android)", () => {
    ...
    describe("getVersionInfo", () => {
      ...
      it("should cache version info after first call", async () => {
        const firstCall = await manager.getVersionInfo();
        const secondCall = await manager.getVersionInfo();
  ...
  describe("Cache Management", () => {
    it("should clear cache and force re-fetch", async () => {
      // First call
      const firstResult = await manager.getVersionInfo();

      // Clear cache
      manager.clearCache();

      // Second call should return a new object (not cached)
      const secondResult = await manager.getVersionInfo();

      // Should be equal in value but different object references
      expect(firstResult).toEqual(secondResult);
      expect(firstResult).not.toBe(secondResult);
    });

    it("should use cached values for multiple calls without cache clear", async () => {
      const versionInfo = await manager.getVersionInfo();
      const version = await manager.getVersion();
      const buildNumber = await manager.getBuildNumber();
      const formattedVersion = await manager.getFormattedVersion();

      // Verify all returned values come from the cached object
      expect(version).toBe(versionInfo.version);
      expect(buildNumber).toBe(versionInfo.buildNumber);
      expect(formattedVersion).toBe(versionInfo.formattedVersion);

      // Verify cache is still working
      const versionInfoAgain = await manager.getVersionInfo();
      expect(versionInfoAgain).toBe(versionInfo);
    });
  });
